    ...
    public Topic upvoteResourceById(@PathParam("id") long resourceId,
        ...
        try {
            ...
            for (AssociationDefinitionModel associationDefinitionModel : typeModel) {
                if (associationDefinitionModel.getChildTypeUri().equals(SCORE_URI)) hasScoreType = true;
            }
            if (hasScoreType) {
                int score = topic.getCompositeValue().getModel().getInt(SCORE_URI) + 1;
                topic.getCompositeValue().set(SCORE_URI, score, clientState, new Directives());
                tx.success();
            } else {
